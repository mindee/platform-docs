...
In both case, pay attention to the [file upload limits](../technical-limitations.md#rate-limits) on the Mindee server. The server will respond with a HTTP 429 code in case of excessive file uploads.
...
Typically, you'll use either raw bytes or a stream object, depending on your language and framework. Send this directly to the Mindee SDK and process as usual (polling or webhook). More information in the section: [#load-a-source-file](load-and-adjust-a-file.md#load-a-source-file "mention")
